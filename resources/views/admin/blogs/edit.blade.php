...
<div class="space-y-6 max-w-5xl">
  ...
  <div class="flex items-center justify-between pb-4 border-b border-slate-800">
    <div class="space-y-1">
      <div class="flex items-center gap-2 text-xs text-slate-400">
        ...
        <span>/</span>
        <span class="text-[#ff3b30] font-bold">Edit Article</span>
      </div>
      <h1 class="text-2xl font-black text-white tracking-tight">Edit: {{ $blog->title }}</h1>
    </div>
    <div class="flex items-center gap-2">
      <a href="{{ route('blogs.show', $blog->slug) }}" target="_blank"
         class="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-slate-800 hover:bg-slate-700 text-white text-xs font-bold transition-colors">
  ...
  <form method="POST" action="{{ route('admin.blogs.update', $blog->id) }}" enctype="multipart/form-data" class="space-y-6">
    ...
    <div class="p-6 rounded-2xl bg-slate-900 border border-slate-800 space-y-5 shadow-xl">
      ...
      <div class="space-y-4">
        ...
        <div class="space-y-1.5">
          ...
          <input type="text" id="titleInput" name="title" value="{{ old('title', $blog->title) }}" required
        ...
        <div class="space-y-1.5">
          ...
          <div class="flex items-center gap-2">
            ...
            <input type="text" id="slugInput" name="slug" value="{{ old('slug', $blog->slug) }}" required
        ...
        <div class="space-y-1.5">
          ...
          <textarea id="excerptInput" name="excerpt" rows="2" required
                    class="w-full px-4 py-2.5 rounded-xl bg-slate-950 border border-slate-800 text-white placeholder-slate-600 focus:outline-none focus:border-[#ff3b30] text-sm leading-relaxed">{{ old('excerpt', $blog->excerpt) }}</textarea>
        </div>
      </div>
    </div>
    ...
    <div class="p-6 rounded-2xl bg-slate-900 border border-slate-800 space-y-5 shadow-xl">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div class="space-y-1.5">
          ...
          <input type="text" id="tagsInput" name="tags" value="{{ old('tags', $tagsString) }}"
                 placeholder="e.g. Laravel, Next.js, FinTech, E-Commerce, Technical SEO"
        ...
        <div class="space-y-1.5">
          <div class="flex items-center justify-between">
            ...
            <span class="px-2 py-0.5 rounded text-[10px] font-semibold bg-blue-500/10 text-blue-400 border border-blue-500/20">
              <i class="fas fa-user-check mr-1"></i> Author
            </span>
          </div>
          <div class="relative">
            <input type="text" id="authorNameInput" name="author_name" value="{{ old('author_name', $blog->author_name) }}" required
                   class="w-full pl-9 pr-4 py-2.5 rounded-xl bg-slate-950 border border-slate-800 text-white focus:outline-none focus:border-[#ff3b30] text-sm">
            <div class="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none text-blue-400">
              <i class="fas fa-shield-alt text-xs"></i>
            </div>
          </div>
          <p class="text-[11px] text-slate-500">Current author: {{ $blog->author_name }}. Change or leave as is.</p>
        </div>
        ...
        <div class="space-y-1.5">
          ...
          <input type="text" id="authorRoleInput" name="author_role" value="{{ old('author_role', $blog->author_role) }}"
                 class="w-full px-4 py-2.5 rounded-xl bg-slate-950 border border-slate-800 text-white focus:outline-none focus:border-[#ff3b30] text-sm">
        </div>
        ...
        <div class="space-y-1.5">
          ...
          <div class="relative">
            <input type="text" id="readTimeInput" name="read_time" value="{{ old('read_time', $blog->read_time) }}" readonly
        ...
        <div class="space-y-3 md:col-span-2 p-4 rounded-xl bg-slate-950 border border-slate-800/80">
          <div class="flex items-center justify-between">
            <div>
              ...
              <p class="text-[11px] text-slate-400">Replace current cover image from device or edit the asset path/URL.</p>
            </div>
          ...
          <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 items-center pt-1">
            ...
            <div class="sm:col-span-1">
              <div class="relative aspect-video rounded-xl overflow-hidden border border-slate-800 bg-slate-900 flex items-center justify-center shadow-inner group">
                <img id="coverPreviewImg" src="{{ asset($blog->featured_image ?: 'asset/blog_icon_bg.png') }}" alt="Cover Preview" class="w-full h-full object-cover">
            ...
            <div class="sm:col-span-2 space-y-3">
              <div>
                <label for="coverFileInput" class="block text-xs font-semibold text-slate-300 mb-1">
                  <i class="fas fa-upload text-[#ff3b30] mr-1"></i> Upload Replacement Image
                </label>
              ...
              <div>
                ...
                <input type="text" id="imageInput" name="featured_image" value="{{ old('featured_image', $blog->featured_image) }}"
                       placeholder="e.g. asset/blog-data-engineering.jpg or uploads/blogs/..."
    ...
    <div class="p-6 rounded-2xl bg-slate-900 border border-slate-800 space-y-5 shadow-xl">
      ...
      <textarea name="content" id="masterBlogContentInput" class="hidden">{{ old('content', $blog->content) }}</textarea>
      ...
      <div id="blogVisualTabWrapper" class="space-y-2">
        <div class="rounded-xl overflow-hidden border border-slate-800 bg-slate-950">
          <textarea id="ckBlogEditor">{{ old('content', $blog->content) }}</textarea>
        </div>
      ...
      <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
        ...
        <div class="p-4 rounded-xl bg-slate-950/70 border border-slate-800 flex items-center justify-between">
          ...
          <label class="relative inline-flex items-center cursor-pointer">
            <input type="checkbox" name="is_published" id="isPublishedToggle" value="1" class="sr-only peer" {{ old('is_published', $blog->is_published) ? 'checked' : '' }}>
        ...
        @php
          $formattedPublishedAt = $blog->published_at ? $blog->published_at->format('Y-m-d\TH:i') : '';
        @endphp
        <div class="p-4 rounded-xl bg-slate-950/70 border border-slate-800 space-y-2">
          <div class="flex items-center justify-between">
            ...
            <span id="scheduleStatusBadge" class="text-[10px] font-bold px-1.5 py-0.5 rounded {{ $blog->is_scheduled ? 'bg-purple-500/20 text-purple-300 border border-purple-500/30' : 'bg-slate-800 text-slate-400' }}">
              {{ $blog->is_scheduled ? 'Scheduled' : ($blog->published_at ? 'Published' : 'Immediate') }}
            </span>
          </div>
          <input type="datetime-local" id="publishedAtInput" name="published_at" value="{{ old('published_at', $formattedPublishedAt) }}"
                 class="w-full px-3 py-1.5 rounded-lg bg-slate-900 border border-slate-800 text-white focus:outline-none focus:border-[#ff3b30] text-xs font-mono">
        ...
        <div class="p-4 rounded-xl bg-slate-950/70 border border-slate-800 flex items-center justify-between">
          <div>
            <p class="text-xs font-bold text-white uppercase tracking-wider">Featured Carousel</p>
            <p class="text-[11px] text-slate-400 mt-0.5">Top spotlight carousel</p>
          </div>
          <label class="relative inline-flex items-center cursor-pointer">
            <input type="checkbox" name="is_featured" value="1" class="sr-only peer" {{ old('is_featured', $blog->is_featured) ? 'checked' : '' }}>
    ...
    <div class="p-6 rounded-2xl bg-slate-900 border border-slate-800 space-y-5 shadow-xl">
      ...
      <div class="space-y-4">
        ...
        <div class="space-y-1.5">
          ...
          <input type="text" id="metaTitleInput" name="meta_title" value="{{ old('meta_title', $blog->meta_title) }}"
                 class="w-full px-4 py-2.5 rounded-xl bg-slate-950 border border-slate-800 text-white placeholder-slate-600 focus:outline-none focus:border-[#ff3b30] text-sm">
        </div>
        ...
        <div class="space-y-1.5">
          ...
          <textarea id="metaDescInput" name="meta_description" rows="2"
                    class="w-full px-4 py-2.5 rounded-xl bg-slate-950 border border-slate-800 text-white placeholder-slate-600 focus:outline-none focus:border-[#ff3b30] text-sm leading-relaxed">{{ old('meta_description', $blog->meta_description) }}</textarea>
        </div>
        ...
        <div class="space-y-1.5">
          ...
          <input type="text" id="keywordsInput" name="focus_keywords" value="{{ old('focus_keywords', $blog->focus_keywords) }}"
                 class="w-full px-4 py-2.5 rounded-xl bg-slate-950 border border-slate-800 text-white placeholder-slate-600 focus:outline-none focus:border-[#ff3b30] text-sm">
        </div>
        ...
        <div class="p-5 rounded-xl bg-slate-950/70 border border-slate-800 space-y-4 pt-4">
          ...
          <div id="faqContainer" class="space-y-3">
            @php
              $currentFaqs = !empty($blog->faqs) && is_array($blog->faqs) ? $blog->faqs : [['question' => '', 'answer' => '']];
            @endphp
        ...
        <div class="p-5 rounded-xl bg-slate-950/70 border border-slate-800 space-y-3">
          ...
          <textarea id="customSchemaInput" name="custom_schema" rows="5" oninput="validateCustomJson()"
                    class="w-full px-4 py-3 rounded-xl bg-slate-900 border border-slate-800 text-emerald-400 placeholder-slate-600 focus:outline-none focus:border-purple-400 text-xs font-mono leading-relaxed">{{ old('custom_schema', $blog->custom_schema) }}</textarea>
        </div>
    ...
    <div class="flex items-center justify-between pt-2">
      <button type="button" onclick="confirmDeleteBlog()"
              class="px-4 py-2.5 rounded-xl bg-red-500/10 hover:bg-red-500/20 text-red-400 border border-red-500/20 font-bold text-xs transition-colors flex items-center gap-2">
  ...
  <form id="deleteBlogForm" method="POST" action="{{ route('admin.blogs.destroy', $blog->id) }}" class="hidden">
    @csrf
    @method('DELETE')
  </form>
...
<script>
  ...
  const blogEditForm = document.querySelector('form');
  if (blogEditForm) {
    blogEditForm.addEventListener('submit', () => {
      if (blogMasterInput) {
