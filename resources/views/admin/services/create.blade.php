...
<div class="max-w-4xl mx-auto space-y-6">
  ...
  <div class="flex items-center justify-between">
    <div class="flex items-center gap-2 text-xs font-semibold text-slate-400">
      ...
      <span>/</span>
      <span class="text-white">Add New Service</span>
    </div>
  ...
  <form method="POST" action="{{ route('admin.services.store') }}" class="space-y-6">
    ...
    <div class="p-6 rounded-2xl bg-slate-900 border border-slate-800 space-y-5 shadow-xl">
      <div class="flex items-center justify-between pb-3 border-b border-slate-800">
        <div>
          ...
          <p class="text-xs text-slate-400">Define the core service name, dynamic category and slug URL.</p>
        </div>
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
        ...
        <div class="space-y-1.5 sm:col-span-2">
          ...
          <input type="text" id="titleInput" name="title" value="{{ old('title') }}" required
                 placeholder="e.g. Headless E-Commerce Architecture"
        ...
        <div class="space-y-1.5 sm:col-span-2">
          <label for="slugInput" class="block text-xs font-bold uppercase tracking-wider text-slate-300">
            URL Slug <span class="text-slate-500 font-normal lowercase">(auto-generated from title if blank)</span>
          </label>
          <div class="flex items-center rounded-xl bg-slate-950 border border-slate-800 overflow-hidden focus-within:border-[#ff3b30]">
            ...
            <input type="text" id="slugInput" name="slug" value="{{ old('slug') }}"
                   placeholder="headless-ecommerce-architecture"
    ...
    <div class="p-6 rounded-2xl bg-slate-900 border border-slate-800 space-y-5 shadow-xl">
      <div class="flex items-center justify-between pb-3 border-b border-slate-800">
        <div>
          ...
          <p class="text-xs text-slate-400">Pick a visual icon, high-converting badge and key performance metric.</p>
        </div>
      ...
      <div class="grid grid-cols-1 sm:grid-cols-2 gap-5">
        ...
        <div class="space-y-2 sm:col-span-2">
          ...
          <div class="flex items-center gap-3">
            <div class="w-12 h-12 rounded-xl bg-slate-950 border border-slate-700 flex items-center justify-center text-[#ff3b30] text-xl shrink-0 shadow-inner" id="iconLivePreview">
              <i class="{{ old('icon', 'fa-solid fa-code') }}"></i>
            </div>
            <input type="text" id="iconInput" name="icon" value="{{ old('icon', 'fa-solid fa-code') }}" required
        ...
        <div class="space-y-1.5 sm:col-span-2">
          ...
          <input type="text" id="taglineInput" name="tagline" value="{{ old('tagline') }}"
                 placeholder="e.g. Next.js App Router, Sub-Second Latency & High Conversion"
                 class="w-full px-4 py-2.5 rounded-xl bg-slate-950 border border-slate-800 text-white placeholder-slate-500 focus:outline-none focus:border-[#ff3b30] text-sm">
        </div>
        ...
        <div class="space-y-1.5 sm:col-span-2">
          ...
          <textarea id="shortDescInput" name="short_description" rows="3"
                    placeholder="Brief 1-2 sentence overview of the service for cards and mega-menu..."
                    class="w-full px-4 py-2.5 rounded-xl bg-slate-950 border border-slate-800 text-white placeholder-slate-500 focus:outline-none focus:border-[#ff3b30] text-sm leading-relaxed">{{ old('short_description') }}</textarea>
        </div>

        <!-- Badge -->
        <div class="space-y-1.5">
          <label for="badgeInput" class="block text-xs font-bold uppercase tracking-wider text-slate-300">
            Highlight Badge <span class="text-slate-500 font-normal lowercase">(e.g. High Performance, GenAI Powered)</span>
          </label>
          <input type="text" id="badgeInput" name="badge" value="{{ old('badge') }}"
        ...
        <div class="grid grid-cols-2 gap-3">
          <div class="space-y-1.5">
            ...
            <input type="text" id="kpiLabelInput" name="kpi_label" value="{{ old('kpi_label') }}"
                   placeholder="e.g. Avg Load Time"
          ...
          <div class="space-y-1.5">
            ...
            <input type="text" id="kpiValueInput" name="kpi_value" value="{{ old('kpi_value') }}"
                   placeholder="e.g. 0.42s or 99.9%"
    ...
    <div class="p-6 rounded-2xl bg-slate-900 border border-slate-800 space-y-5 shadow-xl">
      <div class="flex items-center justify-between pb-3 border-b border-slate-800">
        <div>
          ...
          <p class="text-xs text-slate-400">Bullet points highlighting key deliverables or tech capabilities.</p>
        </div>
      ...
      <div id="featuresContainer" class="space-y-2.5">
        ...
        @endphp
        @foreach($oldFeatures as $fIndex => $feat)
          <div class="feature-row flex items-center gap-2">
            <span class="w-6 text-center text-slate-500 text-xs font-mono">0{{ $fIndex + 1 }}</span>
            <input type="text" name="features[]" value="{{ $feat }}"
                   placeholder="e.g. Edge function caching & microsecond latency"
    ...
    <div class="p-6 rounded-2xl bg-slate-900 border border-slate-800 space-y-5 shadow-xl">
      ...
      <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
        ...
        <div class="space-y-1.5">
          ...
          <input type="number" id="sortOrderInput" name="sort_order" value="{{ old('sort_order', 0) }}" min="0"
        ...
        <div class="p-4 rounded-xl bg-slate-950/70 border border-slate-800 flex items-center justify-between">
          ...
          <label class="relative inline-flex items-center cursor-pointer">
            <input type="checkbox" name="is_featured" value="1" class="sr-only peer" {{ old('is_featured', '1') == '1' ? 'checked' : '' }}>
    ...
    <!-- SECTION 5: SEO, FOCUS KEYWORDS & FAQ RICH SCHEMAS -->
    <div class="p-6 rounded-2xl bg-slate-900 border border-slate-800 space-y-5 shadow-xl">
      <div class="flex items-center justify-between pb-3 border-b border-slate-800">
        <div>
          <h3 class="text-base font-extrabold text-white flex items-center gap-2">
            <span>5. SEO Metadata &amp; FAQ Schema Generator</span>
          ...
          <p class="text-xs text-slate-400">Target high-intent search queries with customized meta tags and structured schema FAQs.</p>
        </div>
      ...
      <div class="space-y-4">
        ...
        <div class="space-y-1.5">
          <div class="flex items-center justify-between">
            ...
            <span id="metaDescCount" class="text-[11px] font-mono text-slate-500">0 / 160</span>
          </div>
        ...
        <div class="space-y-1.5">
          ...
          <input type="text" id="focusKeywordsInput" name="focus_keywords" value="{{ old('focus_keywords') }}"
        ...
        <div id="faqContainer" class="space-y-3">
          <div class="faq-item p-3.5 rounded-xl bg-slate-900 border border-slate-800 space-y-2">
            <div class="flex items-center justify-between gap-2">
              <input type="text" name="faq_questions[]" placeholder="Question: e.g. What is the turnaround time for this service?"
            ...
            <textarea name="faq_answers[]" rows="2" placeholder="Answer: e.g. Typical delivery time ranges between 2 to 4 weeks depending on scope..."
  ...
  "operatingSystem": "All Platforms"
    ...
    <div class="flex items-center justify-end gap-3 pt-2">
      ...
      <button type="submit"
        ...
        <span>Create Service &amp; Generate SEO Page</span>
      </button>
    </div>
  </form>
...
<script>
  ...
  const serviceCreateForm = document.querySelector('form');
  if (serviceCreateForm) {
    serviceCreateForm.addEventListener('submit', () => {
