...
<div class="max-w-5xl mx-auto space-y-6">
    ...
    <div class="flex items-center gap-3">
      ...
      <div>
        <div class="flex items-center gap-2">
          <h1 class="text-2xl font-black text-white tracking-tight">Edit: {{ $industry->name }}</h1>
          <span class="px-2 py-0.5 rounded-full text-[10px] font-bold {{ $industry->is_active ? 'bg-emerald-500/20 text-emerald-400' : 'bg-slate-800 text-slate-400' }}">
            {{ $industry->is_active ? 'Active' : 'Inactive' }}
          </span>
        </div>
        <p class="text-xs text-slate-400 mt-0.5">Slug: <span class="font-mono text-slate-300">/industries/{{ $industry->slug }}</span></p>
      </div>
    </div>
    <div class="flex items-center gap-2">
      <a href="{{ route('industries.show', $industry->slug) }}" target="_blank" class="px-3.5 py-2 rounded-xl bg-slate-800 hover:bg-slate-700 text-slate-300 text-xs font-bold transition-colors flex items-center gap-1.5 border border-slate-700">
        <i class="fas fa-external-link-alt text-[10px]"></i> View Public Page
      </a>
  ...
  <form action="{{ route('admin.industries.update', $industry->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
    @csrf
    @method('PUT')

    <!-- SECTION 1: PRIMARY IDENTITY & CLASSIFICATION -->
    <div class="p-6 rounded-2xl bg-slate-900 border border-slate-800 space-y-5">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
        ...
        <div class="space-y-1.5">
          ...
          <input type="text" name="name" id="industryName" value="{{ old('name', $industry->name) }}" required
        ...
        <div class="space-y-1.5">
          <div class="flex items-center justify-between">
            ...
            <button type="button" onclick="generateSlugFromName()" class="text-[11px] font-semibold text-[#ff3b30] hover:text-red-400 transition-colors flex items-center gap-1 cursor-pointer">
              <i class="fas fa-arrows-rotate text-[10px]"></i> Auto-Generate from Name
            </button>
          </div>
          <div class="flex items-center rounded-xl bg-slate-950 border border-slate-800 overflow-hidden focus-within:border-[#ff3b30] transition-colors">
            ...
            <input type="text" name="slug" id="industrySlug" value="{{ old('slug', $industry->slug) }}" required
        ...
        <div class="space-y-1.5">
          ...
          <div class="flex items-center gap-3">
            ...
            <input type="text" name="icon" id="iconInput" value="{{ old('icon', $industry->icon) }}" required
        ...
        <div class="space-y-1.5">
          ...
          <select name="category_group" class="w-full px-3.5 py-2.5 bg-slate-950 border border-slate-800 rounded-xl text-xs text-slate-200 focus:outline-none focus:border-[#ff3b30]">
            @foreach($categoryGroups as $grp)
              <option value="{{ $grp }}" {{ old('category_group', $industry->category_group) === $grp ? 'selected' : '' }}>{{ $grp }}</option>
            @endforeach
          </select>
        </div>
        ...
        <div class="flex items-center gap-3 pt-6">
          <label class="relative inline-flex items-center cursor-pointer">
            <input type="checkbox" name="is_active" value="1" {{ old('is_active', $industry->is_active) ? 'checked' : '' }} class="sr-only peer">
    ...
    <div class="p-6 rounded-2xl bg-slate-900 border border-slate-800 space-y-5">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
        ...
        <div class="space-y-1.5">
          ...
          <input type="text" name="highlight_stat" value="{{ old('highlight_stat', $industry->highlight_stat) }}"
        ...
        <div class="space-y-1.5">
          ...
          <input type="text" name="stat_label" value="{{ old('stat_label', $industry->stat_label) }}"
        ...
        <div class="md:col-span-2 space-y-1.5">
          ...
          <input type="text" name="hero_tagline" value="{{ old('hero_tagline', $industry->hero_tagline) }}"
        ...
        <div class="md:col-span-2 space-y-1.5">
          ...
          <textarea name="description" rows="3"
                    class="w-full px-3.5 py-2.5 bg-slate-950 border border-slate-800 rounded-xl text-xs text-white focus:outline-none focus:border-[#ff3b30] leading-relaxed">{{ old('description', $industry->description) }}</textarea>
        </div>
        ...
        <div class="space-y-1.5">
          ...
          <input type="text" name="tags" value="{{ old('tags', is_array($industry->tags) ? implode(', ', $industry->tags) : '') }}"
        ...
        <div class="space-y-1.5">
          ...
          <input type="text" name="technologies" value="{{ old('technologies', is_array($industry->technologies) ? implode(', ', $industry->technologies) : 'Next.js 15, Laravel 11, AWS Edge, Redis, GraphQL') }}"
        ...
        <div class="md:col-span-2 space-y-1.5">
          <label class="block text-xs font-bold uppercase tracking-wider text-slate-300">
            Cover / Diagram Image (Optional)
          </label>
          @if(!empty($industry->featured_image))
            <div class="flex items-center gap-3 p-3 bg-slate-950 rounded-xl border border-slate-800 mb-2">
              <img src="{{ $industry->featured_image_url }}" alt="" class="w-16 h-10 object-cover rounded-lg">
              <div class="text-xs">
                <span class="text-slate-300 font-mono block">{{ $industry->featured_image }}</span>
                <span class="text-slate-500 text-[11px]">Upload a new image below to replace</span>
              </div>
            </div>
          @endif
    ...
    <div class="p-6 rounded-2xl bg-slate-900 border border-slate-800 space-y-5 shadow-xl">
      ...
      <textarea name="detailed_content" id="masterDetailedContentInput" class="hidden">{{ old('detailed_content', $industry->detailed_content) }}</textarea>
      ...
      <div id="visualTabWrapper" class="space-y-2">
        <div class="rounded-xl overflow-hidden border border-slate-800 bg-slate-950">
          <textarea id="ckDetailedEditor">{{ old('detailed_content', $industry->detailed_content) }}</textarea>
        </div>
      ...
      <div id="challengeSolutionContainer" class="space-y-4">
        @php
          $challenges = $industry->challenges ?: [
            ['title' => 'Sluggish Time-to-Interactive & Fragile Monoliths', 'description' => 'Legacy CMS templates choke under traffic spikes, causing 8-second cart drops and poor Google rank.'],
            ['title' => 'Compliance Bottlenecks & Security Debt', 'description' => 'Unencrypted data at rest and poorly structured audit trails risk hefty regulatory fines.']
          ];
          $solutions = $industry->solutions ?: [
            ...
            ['title' => 'Zero-Trust Architecture & Automated Auditing', 'description' => 'End-to-end tokenization, OWASP Top 10 hardening, and automated CI/CD security scanning.']
          ];
        @endphp

        @foreach($challenges as $idx => $chal)
          <div class="p-4 rounded-xl bg-slate-950 border border-slate-800 space-y-3 relative group">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div class="space-y-1">
                ...
                <input type="text" name="challenge_titles[]" value="{{ $chal['title'] ?? '' }}" placeholder="Challenge Title" class="w-full px-3 py-2 bg-slate-900 border border-slate-800 rounded-lg text-xs text-white">
      ...
      <div id="faqContainer" class="space-y-3">
        ...
        @foreach($faqs as $faq)
          <div class="p-4 rounded-xl bg-slate-950 border border-slate-800 space-y-2 relative">
            <input type="text" name="faq_questions[]" value="{{ $faq['question'] ?? '' }}" placeholder="Question..." class="w-full px-3 py-2 bg-slate-900 border border-slate-800 rounded-lg text-xs text-white font-bold">
            <textarea name="faq_answers[]" rows="2" placeholder="Answer..." class="w-full px-3 py-2 bg-slate-900 border border-slate-800 rounded-lg text-xs text-slate-300">{{ $faq['answer'] ?? '' }}</textarea>
            <button type="button" onclick="this.closest('.p-4').remove()" class="text-xs text-slate-500 hover:text-red-400 transition-colors">
    ...
    <div class="p-6 rounded-2xl bg-slate-900 border border-slate-800 space-y-5">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
        ...
        <div class="space-y-1.5">
          ...
          <input type="text" name="focus_keywords" value="{{ old('focus_keywords', $industry->focus_keywords) }}" placeholder="e.g. enterprise ecommerce SEO, headless architecture, PCI-DSS compliance" class="w-full px-3.5 py-2.5 bg-slate-950 border border-slate-800 rounded-xl text-xs text-white focus:outline-none focus:border-[#ff3b30]">
        </div>

        <div class="md:col-span-2 space-y-1.5">
          <label class="block text-xs font-bold uppercase tracking-wider text-slate-300">Meta Description</label>
          <textarea name="meta_description" rows="2" placeholder="Compelling 150-160 character description summarizing the vertical solutions..." class="w-full px-3.5 py-2.5 bg-slate-950 border border-slate-800 rounded-xl text-xs text-white focus:outline-none focus:border-[#ff3b30]">{{ old('meta_description', $industry->meta_description) }}</textarea>
        </div>

        <div class="md:col-span-2 space-y-1.5">
          <label class="block text-xs font-bold uppercase tracking-wider text-slate-300">Custom Schema.org JSON-LD (Optional)</label>
          <textarea name="custom_schema" rows="3" placeholder='{"@@context":"https://schema.org","@@type":"Service",...}' class="w-full px-3.5 py-2.5 bg-slate-950 border border-slate-800 rounded-xl text-xs text-white font-mono focus:outline-none focus:border-[#ff3b30]">{{ old('custom_schema', $industry->custom_schema) }}</textarea>
        </div>
      </div>
    </div>
...
<script>
  ...
  function insertArchitectureTemplate() {
    const industryName = document.getElementById('industryName')?.value || 'Sector-Specific';
