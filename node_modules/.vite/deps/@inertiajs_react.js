...
function partition2(collection, predicate = identity) {
  if (!collection) return [[], []];
...
function sampleSize2(collection, size2, guard) {
  ...
  size2 = clamp2(size2, 0, arrayCollection.length);
...
function xorWith2(...values2) {
  const lastValue = last2(values2);
  if (typeof lastValue !== "function") return xor2(...values2);
...
function maxBy2(items, iteratee$1 = identity) {
...
function meanBy2(items, iteratee$1) {
  ...
  return sumBy2(items, iteratee(iteratee$1 ?? identity)) / length;
}
...
function minBy2(items, iteratee$1 = identity) {
  if (items == null) return;
  const array = toArray(items);
  if (array.length === 0) return;
  const getValue = iteratee(iteratee$1);
...
function mergeWithDeep(target, source, merge3, stack) {
  ...
  for (let i = 0; i < sourceKeys.length; i++) {
    ...
    else if (Array.isArray(sourceValue)) target[key] = mergeWithDeep(targetValue, sourceValue, merge3, stack);
    else if (isObjectLike(targetValue) && isObjectLike(sourceValue) && (isPlainObject2(targetValue) || isPlainObject2(sourceValue) || isTypedArray2(targetValue) || isTypedArray2(sourceValue))) target[key] = mergeWithDeep(targetValue, sourceValue, merge3, stack);
    else if (targetValue == null && isPlainObject2(sourceValue)) target[key] = mergeWithDeep({}, sourceValue, merge3, stack);
...
function words2(str, pattern, guard) {
...
function camelCase2(str) {
  const splitWords = words2(normalizeForCase(deburr2(str)));
...
function kebabCase2(str) {
  return words2(normalizeForCase(deburr2(str))).map((word) => word.toLowerCase()).join("-");
}
...
function pad2(str, length = 0, chars = " ") {
...
function snakeCase2(str) {
  return words2(normalizeForCase(deburr2(str))).map((word) => word.toLowerCase()).join("_");
}
...
function startCase2(str) {
  const words$1 = words2(normalizeForCase(deburr2(str)).trim());
  let result2 = "";
...
function upperCase2(str) {
  return words2(normalizeForCase(deburr2(str))).map((word) => word.toUpperCase()).join(" ");
}
...
function createFetchClient(options = {}) {
  ...
  return {
    ...
    async request(config3) {
      ...
      if (config3.headers) {
        Object.entries(config3.headers).forEach(([key, value]) => {
          if (value !== void 0) {
...
var fireClientVisitEvent = (page2, { replace: replace2, visitId }) => {
  return fireEvent("clientVisit", { detail: { page: page2, replace: replace2, visitId } });
};
...
function setHeaders(xhr, config22) {
  ...
  const isFormData2 = isFormDataRequestBody(config22.data);
  Object.entries(config22.headers).forEach(([key, value]) => {
...
var XhrHttpClient = class {
  ...
  doRequest(config22) {
    return new Promise((resolve, reject2) => {
      ...
      const hasRequestedWithHeader = Object.keys(config22.headers ?? {}).some(
      ...
      if (config22.data !== null && config22.data !== void 0) {
        if (isRawRequestBody(config22.data)) {
          body = config22.data;
  ...
  deepMergeObjects(target, source) {
    ...
    for (const key of Object.keys(source)) {
      ...
      if (this.isObject(targetValue) && this.isObject(sourceValue)) {
        result2[key] = this.deepMergeObjects(targetValue, sourceValue);
      } else {
        result2[key] = sourceValue;
      }
    }
    return result2;
  }
...
var Request = class _Request {
  ...
  getHeaders() {
    ...
    const onceProps = Object.entries(page2.onceProps || {}).filter(([, onceProp]) => {
      if (get(page2.props, onceProp.prop) === void 0) {
        return false;
      }
...
var RequestStream = class {
  ...
  cancelInFlight(options = {}) {
    const shouldCancel = typeof options === "function" ? options : (request2) => {
      const { prefetch = true, optimistic = true } = options;
      return (prefetch || !request2.isPrefetch()) && (optimistic || !request2.isOptimistic());
    };
    this.requests.filter(shouldCancel).forEach((request2) => request2.cancel({ cancelled: true }));
  }
...
var Router = class {
  ...
  visit(href, options = {}) {
    ...
    if (!isSamePage) {
      this.asyncRequestStream.cancelInFlight(
        (request2) => !request2.isPrefetch() && !request2.isOptimistic() && isSameUrlWithoutQueryOrHash(request2.getUrl(), currentPageUrl)
      );
    }
    if (!visit.async) {
  ...
  preserveOncePropsOnInstantVisit(current, props) {
    ...
    Object.entries(current.onceProps ?? {}).forEach(([key, onceProp]) => {
      if (get(props, onceProp.prop) !== void 0) {
        return;
      }
  ...
  applyOptimisticUpdate(optimistic, events) {
    ...
    for (const key of changedKeys) {
      page.setBaseline(key, cloneDeep(currentProps[key]));
    }
...
var useInfiniteScrollElementManager = (options) => {
  ...
  const restoreElements = () => {
    ...
    for (let index = 0; index < childNodes.length; index++) {
      ...
      const element = node;
      let elementPage;
      for (const [page2, range3] of Object.entries(pageElementRange)) {
...
function normalizeLayouts(layout, isComponent2, isRenderFunction2) {
  if (!layout || isRenderFunction2 && isRenderFunction2(layout)) {
    return [];
  }
  if (isNamedLayouts(layout, isComponent2)) {
    return Object.entries(layout).map(([name, value]) => ({ ...extract(value, isComponent2), name }));
  }
  if (isTuple(layout, isComponent2)) {
    return [{ component: layout[0], props: layout[1] ?? {} }];
  }
  if (Array.isArray(layout)) {
    return layout.map((item) => extract(item, isComponent2));
  }
  if (hasComponentKey(layout) && isComponent2(layout.component)) {
    return [{ component: layout.component, props: layout.props ?? {} }];
  }
  if (isComponent2(layout)) {
...
// node_modules/@inertiajs/react/dist/index.js
...
function App({
  ...
  const renderChildren = children || (({ Component, props, key }) => {
    ...
    if (isLayoutResolver(layoutValue)) {
      const result2 = layoutValue(props);
      if ((0, import_react.isValidElement)(result2)) {
...
async function createInertiaApp({
  ...
  const wrapWithStrictMode = (element) => {
    return strictMode ? (0, import_react4.createElement)(import_react4.StrictMode, null, element) : element;
  };
  const resolveComponent = (name, page22) => Promise.resolve(resolve(name, page22)).then((module) => {
    return module.default || module;
  });
  if (isServer3 && !page2 && !render2) {
    return async (page22, renderToString) => {
      let head22 = [];
      const initialComponent = await resolveComponent(page22.component, page22);
      const props = {
        initialPage: page22,
        initialComponent,
        resolveComponent,
      ...
      if (setup) {
        ...
        reactApp2 = wrapWithStrictMode((0, import_react4.createElement)(App, props));
        if (withApp) {
          reactApp2 = withApp(reactApp2, { ssr: true, page: page22 });
        }
      }
      const html = renderToString(reactApp2);
      const body = buildSSRBody(id, page22, html);
  ...
  const reactApp = await Promise.all([
    ...
    let appElement = wrapWithStrictMode((0, import_react4.createElement)(App, props));
    if (withApp) {
      appElement = withApp(appElement, { ssr: false, page: initialPage });
    }
    if (el.hasAttribute("data-server-rendered")) {
      (0, import_client3.hydrateRoot)(el, appElement);
    } else {
      (0, import_client3.createRoot)(el).render(appElement);
    }
  });
  if (!isServer3 && progress22) {
    setupProgress(progress22);
  }
  if (isServer3 && render2 && reactApp) {
    const html = render2(reactApp);
    const body = buildSSRBody(id, initialPage, html);
    return { head: head3, body };
  }
}
function usePage() {
  const page2 = (0, import_react6.use)(PageContext_default);
  if (!page2) {
...
var Deferred = ({ children, data, rescue, fallback }) => {
  ...
  const [reloading, setReloading] = (0, import_react5.useState)(false);
  const activeReloads = (0, import_react5.useRef)(/* @__PURE__ */ new Set());
  const page2 = usePage();
  const pageProps = page2.props;
  const keys2 = (0, import_react5.useMemo)(() => Array.isArray(data) ? data : [data], [data]);
  const rescuedKeys = (0, import_react5.useMemo)(() => new Set(page2.rescuedProps), [page2.rescuedProps]);
  (0, import_react5.useEffect)(() => {
    const removeStartListener = router.on("start", (e) => {
      const visit = e.detail.visit;
      if (visit.preserveState === true && isSameUrlWithoutQueryOrHash(visit.url, window.location) && partialReloadRequestsSomeProps(visit, keys2)) {
  ...
  const propsAreDefined = (0, import_react5.useMemo)(() => keys2.every((key) => get(pageProps, key) !== void 0), [keys2, pageProps]);
  const hasRescuedProps = (0, import_react5.useMemo)(() => keys2.some((key) => rescuedKeys.has(key)), [keys2, rescuedKeys]);
...
function useFormState(options) {
  ...
  const precognitionEndpointRef = (0, import_react9.useRef)(options.precognitionEndpoint ?? null);
  const [defaults2, setDefaultsState] = (0, import_react9.useState)(cloneDeep(initialData));
  const [data, setData] = useDataState ? useDataState() : (0, import_react9.useState)(cloneDeep(initialData));
  const [errors, setErrors] = useErrorsState ? useErrorsState() : (0, import_react9.useState)({});
  ...
  const transformRef = (0, import_react9.useRef)((data2) => data2);
  const defaultsCalledInOnSuccessRef = (0, import_react9.useRef)(false);
  ...
  const withAllErrorsRef = (0, import_react9.useRef)(null);
  const withAllErrorsEnabled = () => withAllErrorsRef.current ?? config2.get("form.withAllErrors");
  ...
  const markAsSuccessful = (0, import_react9.useCallback)(() => {
    clearErrors();
  ...
  const resetBeforeSubmit = (0, import_react9.useCallback)(() => {
    setWasSuccessful(false);
    setRecentlySuccessful(false);
    clearTimeout(recentlySuccessfulTimeoutId.current);
  }, [setWasSuccessful, setRecentlySuccessful]);
  const finishProcessing = (0, import_react9.useCallback)(() => {
    setProcessing(false);
    setProgress(null);
  ...
  const form = {
    data,
    isDirty: !isEqual(data, defaults2),
    errors,
  ...
  const withPrecognition = (...args) => {
    ...
    if (!validatorRef.current) {
      const validator = createValidator(
        (client2) => {
          const { method: method2, url } = precognitionEndpointRef.current();
...
function useForm(...args) {
  ...
  const pendingOptimisticRef = (0, import_react8.useRef)(null);
  ...
  const submit = (0, import_react8.useCallback)(
    (...args2) => {
      const { method: method2, url, options } = UseFormUtils.parseSubmitArguments(args2, precognitionEndpointRef.current);
  ...
  const submitMethods = (0, import_react8.useMemo)(
    () => ({
...
var Form = (0, import_react7.forwardRef)(
  ({
    ...
    const resolvedComponent = (0, import_react7.useMemo)(() => {
      if (component) {
        return component;
      }
    ...
    const cancelOnUnmountRef = (0, import_react7.useRef)(cancelOnUnmount);
    ...
    const submit = (submitter) => {
      ...
      const submitOptions = {
        ...
        onSuccess: async (...args) => {
          const result2 = await onSuccess(...args);
          onSubmitComplete({
            reset,
            defaults: defaults2
          });
...
function useFormContext() {
  return (0, import_react7.use)(FormContext);
}
var Form_default = Form;
var Head = function({ children, title }) {
  const headManager = (0, import_react11.use)(HeadContext_default);
...
var Link = (0, import_react13.forwardRef)(
  ({
    ...
    const resolvedComponent = (0, import_react13.useMemo)(() => {
      if (component) {
        return component;
      }
...
function useHttp(...args) {
  const { rememberKey, data, precognitionEndpoint } = UseFormUtils.parseUseFormArguments(...args);
  const initialDefaults = typeof data === "function" ? cloneDeep(data()) : cloneDeep(data);
  const abortController = (0, import_react14.useRef)(null);
  const excludeKeysRef = (0, import_react14.useRef)([]);
  const pendingOptimisticRef = (0, import_react14.useRef)(null);
  const [response, setResponse] = (0, import_react14.useState)(null);
  const useDataState = rememberKey ? () => useRemember(initialDefaults, `${rememberKey}:data`, excludeKeysRef) : void 0;
  const useErrorsState = rememberKey ? () => useRemember({}, `${rememberKey}:errors`) : void 0;
  const {
    form: baseForm,
  ...
  const submit = (0, import_react14.useCallback)(
    async (method2, url, options) => {
      ...
      try {
        ...
        throw new HttpResponseError2(`Request failed with status ${httpResponse.status}`, httpResponse);
      } catch (error) {
  ...
  const submitWithArgs = (0, import_react14.useCallback)(
    (...args2) => {
  ...
  const submitMethods = (0, import_react14.useMemo)(
    () => ({
...
function usePoll(interval, requestOptions = {}, options = {
  ...
  const latest = (0, import_react15.useRef)(requestOptions);
  latest.current = requestOptions;
  const pollRef = (0, import_react15.useRef)(null);
  const [polling, setPolling] = (0, import_react15.useState)(options.autoStart ?? true);
  ...
  const start2 = (0, import_react15.useCallback)(() => {
...
var WhenVisible = ({ children, data, params, buffer, as, always, fallback }) => {
  ...
  const keys2 = (0, import_react17.useMemo)(() => data ? Array.isArray(data) ? data : [data] : [], [data]);
  const [loaded, setLoaded] = (0, import_react17.useState)(() => keys2.length > 0 && keys2.every((key) => get(pageProps, key) !== void 0));
  ...
  (0, import_react17.useEffect)(() => {
    if (keys2.length > 0) {
      setLoaded(keys2.every((key) => get(pageProps, key) !== void 0));
    }
  }, [pageProps, keys2]);
